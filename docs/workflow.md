...
| PR merged on GitHub, local branch/STATE left over | `/flow-finish` → sync after remote merge |
...
**Gitignore it** in consumer projects. Specs, plans, patches, and brainstorms are the durable record — STATE is ephemeral (phase churn, worktree paths). Flow offers to add `docs/flow/STATE.md` to `.gitignore` before the first write (`state-setup.md`). Opt out explicitly if you want STATE in git history.
