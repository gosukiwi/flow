...
6. If the plan maps Success Criteria unlocked by this task, the orchestrator evidences each now (pass/fail with observed behavior). Fail → stop; do not start Task N+1. The reviewer does not check Success Criteria — unlock checks and final Verify are the orchestrator's.
