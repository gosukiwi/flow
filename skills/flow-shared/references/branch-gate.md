...
| On feature branch, unrelated new work | Offer **1) switch branch here** (warn: disrupts WIP) or **2) worktree** (recommended). Stop until confirmed. |
...
- **Before creating or switching branches or worktrees:** stop and ask the user.
  ...
  - When on `main`/`master` or unrelated work on a feature branch, also offer workspace option (in-place vs worktree)
...
### Unrelated work on existing feature branch
...
- Mix unrelated commits onto a branch with active WIP for a different topic
