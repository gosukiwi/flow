...
| `flow-spec-spec-gate-menu-expectations.md` | `/flow-spec` | Spec gate option 1 sets post-approval expectations — subagents after branch, no Phase B / misleading "no code" |
...
| `flow-execute-fix-then-start-task2.md` | `/flow-execute` | After correctness-review fix commit, do not start Task 2 until Task 1 spec + correctness rerun |
...
| `flow-execute-midplan-review-bypass.md` | `/flow-execute` | Agent does not skip correctness or parallel Task N+1 after mid-plan spec ✅ |
...
| `flow-patch-workspace-numbered-menu.md` | `/flow-patch` | After micro-spec approval on done-lane feature branch → numbered workspace gate only; no freeform confirm phrases |
