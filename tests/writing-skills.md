...
| `flow-spec-skip-progress-update.md` | `/flow-spec` | **B** — progress line after each subagent return (task N of M); "don't interrupt" does not skip it (RED + GREEN on Grok 4.5 High) |
| `flow-spec-skip-kickoff-progress.md` | `/flow-spec` | **B** — progress line before the first dispatch (Task 1 of M — starting implementer); "don't interrupt" does not skip it (RED + GREEN on Grok 4.5 High) |
...
| `flow-spec-monolith-spec.md` | `/flow-spec` | **B** — split work that exceeds one slice into separate specs (RED on Opus 5; GREEN on Grok 4.5 High historically; GREEN re-run on Grok 4.5 after the slice-rule rewrite) |
| `flow-spec-oversized-single-feature.md` | `/flow-spec` | **B** — one coherent subsystem still splits when it exceeds one session / one shippable slice; spec the first slice only (RED + GREEN on Opus 5 and on Grok 4.5) |
| `flow-spec-scope-not-trimmed.md` | `/flow-spec` | **B** — anything not required for the slice to ship and pass its own tests goes to Out of Scope, however small; not Scope and not Design follow-ups (RED + GREEN on Opus 5 and on Grok 4.5; GREEN re-confirmed on both after the bordering-slice rewrite) |
| `flow-spec-bordering-slice-unfenced.md` | `/flow-spec` | **B** — fence deferred work bordering this slice in Out of Scope even with zero file overlap; not every deferred slice, which would smuggle in a roadmap (RED on Grok 4.5 — compliant on Opus 5 across two sharpenings, so it REDs only at the large tier; GREEN on Opus 5 and Grok 4.5) |
